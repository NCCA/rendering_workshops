...
BumpScene::BumpScene() : Scene() {}


void BumpScene::initGL() noexcept {
    ...
    shader->loadShader("BumpProgram",
                       "shaders/bump_vert.glsl",
                       "shaders/bump_frag.glsl");
    (*shader)["BumpProgram"]->use();

    // Load up our textures
    initTexture(0, m_colourTex, "images/facetexture.jpg");
    initTexture(1, m_normalTex, "images/facenormals.jpg");
...
void BumpScene::initTexture(const GLuint& texUnit, GLuint &texId, const char *filename) {
...
void BumpScene::paintGL() noexcept {
